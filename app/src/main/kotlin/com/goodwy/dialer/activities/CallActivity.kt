...
import com.goodwy.dialer.api.RetrofitClient
...
import com.goodwy.dialer.requests.ProofRequest
import com.goodwy.dialer.utils.SharedPreferencesUtils
...
class CallActivity : SimpleActivity() {
    companion object {
        fun getStartIntent(context: Context, needSelectSIM: Boolean = false): Intent {
            ...
            openAppIntent.flags =
                Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_REORDER_TO_FRONT //Intent.FLAG_ACTIVITY_BROUGHT_TO_FRONT --removed it, it can cause a full screen ringing instead of notifications
    ...
    private var needSelectSIM =
        false //true - if the call is called from a third-party application not via ACTION_CALL, for example, this is how MIUI applications do it.
    private var isVerifyingCaller = false
    private var isCallerVerified = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.verifyingCallerText?.beGone()
        binding.verifyingProgress?.beGone()
        ...
        if (configBackgroundCallScreen == TRANSPARENT_BACKGROUND || configBackgroundCallScreen == BLACK_BACKGROUND ||
            ...
            binding.apply {
                arrayOf(
                    ...
                    dialpadClose, callEndLabel, callAcceptAndDecline, verifyingCallerText
                ).forEach {
    ...
    private fun updateVerificationUI() {
        binding.apply {
            if (isVerifyingCaller) {
                verifyingCallerText?.beVisible()
                verifyingProgress?.beVisible()
                verifyingCallerText?.setTextColor(getColor(R.color.md_yellow_700))
            } else if (isCallerVerified) {
                verifyingProgress?.beGone()
                verifyingCallerText?.setTextColor(getColor(R.color.md_blue))
                verifyingCallerText?.text = getString(R.string.verified_caller)
                verifiedCallerImage?.beVisible()
            } else {
                verifyingCallerText?.beGone()
                verifyingProgress?.beGone()
                verifiedCallerImage?.beGone()
            }
        }
    }
...
//                openBluetoothSettings()
    ...
    private fun handleSwipe() = binding.apply {
        ...
        callAccept.onGlobalLayout {
            ...
            initialDraggableX =
                if (isRtl) callDraggableBackground.right.toFloat() - callDraggable.width.toFloat() else callDraggableBackground.left.toFloat() + resources.getDimension(
                    R.dimen.three_dp
                )
        ...
        val colorBg =
            if (configBackgroundCallScreen == TRANSPARENT_BACKGROUND || configBackgroundCallScreen == BLUR_AVATAR || configBackgroundCallScreen == AVATAR || configBackgroundCallScreen == BLACK_BACKGROUND) Color.WHITE
...
//                    }
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    callDraggableBackground.layoutParams.width =
                        if (isRtl) (initialBackgroundWidth - (maxDragX + 60 - callDraggable.height.toFloat() - callDraggable.x)).toInt()
...
//    }
    ...
    fun getAllConnectionsAndVerify(number: String) {
        val token = "Bearer ${SharedPreferencesUtils.getToken(this)}}"
        RetrofitClient.apiService.getAllConnections(token, "test")
            .enqueue(object : Callback<ConnectionResponse> {
                override fun onResponse(call: retrofit2.Call<ConnectionResponse>, response: Response<ConnectionResponse>) {
                    if (response.isSuccessful) {
                        val fetchedConnections = response.body()!!.response
                        SharedPreferencesUtils.saveConnections(this@CallActivity, fetchedConnections)
                        matchAndSendProof(fetchedConnections, number)

                    } else {
                        Log.d("acceptConnection api ->", " Error: ${response.code()}")
                        Toast.makeText(this@CallActivity, "Error: ${response.code()}", Toast.LENGTH_SHORT).show()
                    }
                }

                override fun onFailure(call: retrofit2.Call<ConnectionResponse>, t: Throwable) {
                    Log.d("acceptConnection api ->", " Error: ${t.message}")
                    Toast.makeText(this@CallActivity, "Failed: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
    }

    private fun verifyCaller(number: String) {
        val connections = SharedPreferencesUtils.getConnections(this)

        if (connections.isEmpty()) {
            // Fetch connections first, then verify
            getAllConnectionsAndVerify(number)
        } else {
            matchAndSendProof(connections, number)
        }
    }

    private fun matchAndSendProof(connections: List<Connection>, number: String) {
        val matchedConnection = connections.firstOrNull { it.theirLabel == number.removeNumberFormatting() }

        if (matchedConnection != null) {
            isVerifyingCaller = true
            isCallerVerified = false
            updateVerificationUI()
            sendProofRequest(matchedConnection.id!!)
        } else {
            Toast.makeText(this, "Caller not found in connections", Toast.LENGTH_SHORT).show()
        }
    }

    private fun sendProofRequest(connectionId: String) {
        val proofRequest = ProofRequest(connectionId = connectionId)
        val token = "Bearer ${SharedPreferencesUtils.getToken(this)}}"
        RetrofitClient.apiService.sendProofRequest(token, proofRequest)
            .enqueue(object : Callback<ApiResponse<ProofDetail>> {
                override fun onResponse(call: retrofit2.Call<ApiResponse<ProofDetail>>, response: Response<ApiResponse<ProofDetail>>) {
                    if (response.isSuccessful) {

                        response.body()?.let {
                            getProofRequestStatus(it.response.proofExchangeRecord.id!!)
                            startPolling(it.response.proofExchangeRecord.id)
...
//                        getAllConnection()
                ...
                override fun onFailure(call: retrofit2.Call<ApiResponse<ProofDetail>>, t: Throwable) {
                    Log.d("acceptConnection api ->", " Error: ${t.message}")
                    Toast.makeText(this@CallActivity, "Failed: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
    ...
    fun getProofRequestStatus(proofRecordId: String) {

        val token = "Bearer ${SharedPreferencesUtils.getToken(this)}}"
        RetrofitClient.apiService.getProofRequestStatus(token, "test", proofRecordId)
            .enqueue(object : Callback<ProofRequestStatusResponse> {
                override fun onResponse(call: retrofit2.Call<ProofRequestStatusResponse>, response: Response<ProofRequestStatusResponse>) {
                    if (response.isSuccessful) {

                        response.body().let {
                            if (it!!.status) {
                                isVerifyingCaller = false
                ...
                override fun onFailure(call: retrofit2.Call<ProofRequestStatusResponse>, t: Throwable) {
                    Log.d("acceptConnection api ->", " Error: ${t.message}")
                    Toast.makeText(this@CallActivity, "Failed: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            })
    ...
    private fun startPolling(proofRecordId: String) {
        isPolling = true
        handler.postDelayed(object : Runnable {
            override fun run() {
                if (isPolling) {
                    getProofRequestStatus(proofRecordId)
    ...
    private fun getCallerNumber(call: Call?): String {
        return call?.details?.handle?.let { uri ->
            // Extract phone number from the URI (e.g., "tel:+1234567890")
    ...
    private fun updateCallState(call: Call) {
        ...
        binding.apply {
            ...
            getCallContact(applicationContext, call) { callContact ->
                // This callback runs when the contact lookup is complete
                // Use the callContact object here
                val callerName = callContact.name
                val callerNumber = callContact.number
                val photoUri = callContact.photoUri

                if (call.getStateCompat() == Call.STATE_RINGING) {
                    verifyCaller(callerNumber)
                }
            }
